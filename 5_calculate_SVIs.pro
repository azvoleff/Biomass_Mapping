PRO export_predictors
  COMPILE_OPT idl2, hidden
  
  TIC
  
  input_folder = "M:\Data\Nepal\Imagery\Biomass_Mapping\5_stacked"
  output_path = "M:\Data\Nepal\Imagery\Biomass_Mapping\6_layers_for_biomass_prediction"
  
  bgrn_images = FILE_SEARCH(input_folder + PATH_SEP() + $
    "east_*_bgrn_warp2pan_atmosc.envi")
  pan_images = FILE_SEARCH(input_folder + PATH_SEP() + $
    "east_*_pan_*_atmosc.envi")
  ...
  FOR i=0L, (N_ELEMENTS(bgrn_images)-1) DO BEGIN
    file = bgrn_images[i]
    bgrn_images_clock = TIC('File: ' + file)
    ...
    ; Calculate NDVI
    ndvi_out_name = output_path + PATH_SEP() + year + "_ndvi.dat"
    ...
    ; Calculate EVI  
    evi_out_name = output_path + PATH_SEP() + year + "_evi.dat"
    ...
    msavi_out_name = output_path + PATH_SEP() + year + "_msavi.dat"
    ...
    ENVI_DOIT,'MATH_DOIT', FID=[fid, fid], POS=[2L, 3L], DIMS=dims, $
     ...
     ; Calculate ARVI
     arvi_out_name = output_path + PATH_SEP() + year + "_arvi.dat"
